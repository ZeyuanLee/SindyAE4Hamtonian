...
# ╔═╡ 712d6dcb-c1c0-46cf-a565-714ecc45d416
using MLJ

# ╔═╡ 5ac3116d-26e6-47b8-9aa4-fb44b8278419
using DelimitedFiles

# ╔═╡ 5b3a5f34-bcbb-47ea-b533-2c102cc5dda3
using SymbolicRegression

# ╔═╡ 5263b4f7-c90a-4306-959b-93cd6d782e0e
using SymbolicRegression: ValidVector

# ╔═╡ 7846a3cf-c815-4ac5-821a-fbc519bcac35
using MLJBase: machine, fit!, predict, report

# ╔═╡ 2fd67f48-c1ba-492a-a8ce-2309a7a4d258
using SymbolicUtils

# ╔═╡ a19e5fc7-1bb8-4879-9df8-0db50f148310
using Plots

# ╔═╡ 767ec598-6353-4bee-8c38-42190b956f0c
# ╠═╡ disabled = true
#=╠═╡
MultitargetSRRegressor = @load MultitargetSRRegressor pkg=SymbolicRegression
  ╠═╡ =#

# ╔═╡ 34401f3d-1d7d-4bcf-b6da-a5da774cedc2
# 配列の読み込み

# ╔═╡ 04f28a47-a001-450f-992c-4df3abc855a5
# ╠═╡ disabled = true
#=╠═╡
qm = readdlm("exbl_x.txt")
  ╠═╡ =#

# ╔═╡ 89436d49-7ef0-4abb-9cde-41f0493fbe7e
# 行列形式では扱えない

# ╔═╡ 34ae818e-be1c-485e-bf6f-6b46010220bf
for i in 1:4  # ← 読み込むファイル数に応じて N を指定
    filename = "exbl_x$(i).txt"
    qm = readdlm(filename)
    append!(q, [Tuple(qm[j, :]) for j in 1:size(qm, 1)])
end

# ╔═╡ b4c5812e-24df-4940-aa07-68f1dac349d5
# ╠═╡ disabled = true
#=╠═╡
q = transpose(qt)
  ╠═╡ =#

# ╔═╡ 6270b636-75e3-4ea6-a8ff-cdbcfe7fb919
size(q)

# ╔═╡ ed67e3f9-e96c-401a-b9e1-d99f567edbb6
N = size(q,1)

# ╔═╡ 72c5a63c-94b7-40ce-9f70-29a1610508d4
# ╠═╡ disabled = true
#=╠═╡
pm = readdlm("exbl_p.txt")
  ╠═╡ =#

# ╔═╡ 11105c59-d368-465a-8fe2-a76c7441f9eb
for i in 1:4  # ← 読み込むファイル数に応じて N を指定
    filename = "exbl_v$(i).txt"
    pm = readdlm(filename)
    append!(p, [Tuple(pm[j, :]) for j in 1:size(pm, 1)])
end

# ╔═╡ 6fc30f51-8aba-4320-84b1-a89b38e86c77
# ╠═╡ disabled = true
#=╠═╡
p = transpose(pt)
  ╠═╡ =#

# ╔═╡ 5bc66386-7c41-47fe-92ee-6c35e43c9261
# 入力層の指定=各時間微分

# ╔═╡ d73bdfb5-8f4c-463a-a9b8-d3ee20ced03b
qdotm = zeros(3,N)

# ╔═╡ 30fa529f-4ef0-4d58-af4e-01bc420be2a6
#=╠═╡
for i in 1:N-1
	qdotm[i,:] = qm[i+1,:] - qm[i,:]
end
...
# ╔═╡ e8025552-ec7e-40f6-a666-92c8e994bdb8
for i in 1:N-1
...
# ╔═╡ 719a3dc2-6524-4031-9e05-c106ae190174


# ╔═╡ 8a829210-0145-4b0b-912d-2d0bcf2162b0
qdot = [Tuple(qdotm[:,i]) for i in 1:N]

# ╔═╡ 6426b290-2fa5-4f02-a961-ac8b30bf8cac
pdotm = zeros(3,N)

# ╔═╡ e89fc96d-388d-4be1-990b-8b10906ab8c0
#=╠═╡
for i in 1:N-1
	pdotm[i,:] = pm[i+1,:] - pm[i,:]
end
...
# ╔═╡ dcb90b77-b81b-400c-a2b4-49eb242ccbe4
for i in 1:N-1
...
# ╔═╡ f98355b0-c355-4627-92a6-5d10b8257ce9
pdot = [Tuple(pdotm[:,i]) for i in 1:N]

# ╔═╡ a7eb5893-96c5-4b85-a43f-e1bca851da11
# ╠═╡ disabled = true
#=╠═╡
dot = zeros(N,6)
  ╠═╡ =#

# ╔═╡ 674f7b69-32e2-4bf3-be6a-f4421514d85c
# ╠═╡ disabled = true
#=╠═╡
co = [(q..., p...) for (q,p) in zip(qdot, pdot)]
  ╠═╡ =#

# ╔═╡ 607c0d18-546c-4a23-99f5-85b23afcfed5
co = [(q[1], q[2], p[1], p[2]) for (q,p) in zip(qdot, pdot)]

# ╔═╡ 690fd3fd-71e8-4201-b953-4928feba1fd2
# ╠═╡ disabled = true
#=╠═╡
for i in 1:N-1
	for j = 1:3
...
# ╔═╡ 896c6765-c8e3-4185-b36e-df464837c865
data = (; q, p, qdot, pdot, co)

# ╔═╡ e809b5bc-557b-458c-bc01-93d59106af01
# ╠═╡ disabled = true
#=╠═╡
X = (;
    q_x=q[1,1],
...
# ╔═╡ a0a94901-5601-461d-8b5d-1eac34f20769
X = (;
...
# ╔═╡ 09c0a2e3-fcb1-4680-a73a-81f7a89423a4
keys(X)

# ╔═╡ 48a8d1d3-6e4b-4dc0-8b0c-41c8ccfa5455
# ╠═╡ disabled = true
#=╠═╡
y = (;
    qdot_x=qdot[1,1],
...
# ╔═╡ 3744ece3-1c8a-4ab5-933f-ab2756294efa
# ╠═╡ disabled = true
#=╠═╡
y = (;
    qdot_x=qdot[:,1],
...
# ╔═╡ 4d858499-dcc4-4c69-9efe-f6992c76aedc
# ╠═╡ disabled = true
#=╠═╡
y = hcat(
    qdot[:,1],
...
# ╔═╡ a7490c28-fc06-48b4-b597-b4f4beeb6b4e
struct DC{T}
	qdot_x::T
	qdot_y::T
	# qdot_z::T
...
# ╔═╡ 6a2af34e-6f89-4ea5-93ea-32b3ab4bb6e2
y = [DC(co...) for co in data.co]

# ╔═╡ 1732bb45-278a-47ee-81a5-465c59781cc5
variable_names = ["q_x", "q_y", "q_z", "p_x", "p_y", "p_z"]

# ╔═╡ f1883c58-4ba0-4e6f-9763-ac5bef25b909
# 微分方程式の形の指定(ハミルトニアンの定義)

# ╔═╡ 0a83556b-06d8-4df3-87ef-7fdcfe49be1c
...
scatter(
    ...
    [yp.x.qdot_x for yp in ypred],   # 予測値（.x は ValidVector の中身）
    xlabel = "True q̇ₓ",
...
# ╔═╡ 4d28229f-5a09-4494-a96e-e43ccc26216f
# ╠═╡ disabled = true
#=╠═╡
p = [Tuple(pm[i, :]) for i in 1:size(pm, 1)]
  ╠═╡ =#

# ╔═╡ 7892792c-10e3-4258-b7b4-f52c85d0d3ce
q = []

# ╔═╡ 9c282db9-fa23-4411-8342-68b606994fe8
# ╠═╡ disabled = true
#=╠═╡
q = [Tuple(qm[i, :]) for i in 1:size(qm, 1)]
  ╠═╡ =#

# ╔═╡ 08e908f7-9bf2-49e7-94fa-c3e6498a788e
p = []
...
manifest_format = "2.0"
project_hash = "4551c3b300845bfde23e6136e4af36411f199e9d"
...
git-tree-sha1 = "82072a3f7bf353fbdf85bb8bf32562961c97b23b"
...
git-tree-sha1 = "11582404a6e7ffcf889a9b2699784edc780ec38e"
